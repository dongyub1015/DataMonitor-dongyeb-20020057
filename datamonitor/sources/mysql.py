...
class MySQLSource(DataSource):
    ...
    def _build_url(self) -> str:
        ...
        user = c.user or ""
        pwd = c.password or ""
        return f"mysql+pymysql://{user}:{pwd}@{c.host}:{port}/{c.database}"
    ...
    async def fetch_metrics(self) -> list[MetricSnapshot]:
        ...
        for watch in self.config.watch:
            ...
            snap = await asyncio.to_thread(
                self._fetch_table, watch.table, watch.exact_count
            )
            results.append(snap)
        return results

    def _fetch_table(self, table: str, exact_count: bool) -> MetricSnapshot:
        try:
            _safe_table(table)
            with self._engine.connect() as conn:  # type: ignore[union-attr]
                if exact_count:
                    ...
                    row_count = conn.execute(
                        ...
                        {"db": self.config.database, "tbl": table},
                    ).scalar() or 0
            ...
            return MetricSnapshot(
                ...
                metrics={"row_count": int(row_count), "size_mb": float(size_mb)},
            )
